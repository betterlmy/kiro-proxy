...
func responsesResponse(id, model string, source map[string]any, previousID string) map[string]any {
	output := responseOutput(source)
	return map[string]any{
...
func responseOutput(source map[string]any) []any {
	...
	for _, raw := range tools {
		...
		output = append(output, map[string]any{"id": "fc_" + idSuffix(), "type": "function_call", "call_id": tool["id"], "name": fn["name"], "arguments": fn["arguments"], "status": "completed"})
	}
	return output
}
...
func newStreamWriter(w http.ResponseWriter, f http.Flusher, api surface, id, model, previousID string) *streamWriter {
	return &streamWriter{w: w, f: f, api: api, id: id, model: model, previousID: previousID}
}
...
func (s *streamWriter) delta(delta respconv.EventDelta) {
	...
	if delta.ToolStop {
		...
		if s.api == surfaceChat {
			...
			itemID := "fc_" + delta.ToolUseID
			s.event(map[string]any{"type": "response.output_item.added", "output_index": s.responseOutputIndex, "item": map[string]any{"id": itemID, "type": "function_call", "call_id": delta.ToolUseID, "name": delta.ToolName, "arguments": "", "status": "in_progress"}})
			s.event(map[string]any{"type": "response.function_call_arguments.delta", "output_index": s.responseOutputIndex, "item_id": itemID, "delta": delta.ToolInput})
			s.event(map[string]any{"type": "response.function_call_arguments.done", "output_index": s.responseOutputIndex, "item_id": itemID, "arguments": delta.ToolInput})
			s.event(map[string]any{"type": "response.output_item.done", "output_index": s.responseOutputIndex, "item": map[string]any{"id": itemID, "type": "function_call", "call_id": delta.ToolUseID, "name": delta.ToolName, "arguments": delta.ToolInput, "status": "completed"}})
			s.responseOutputIndex++
		}
	}
}
...
func (s *streamWriter) finish(source map[string]any, includeUsage bool) {
	...
	s.event(map[string]any{"type": "response.completed", "response": responsesResponse(s.id, s.model, source, s.previousID)})
}
